...
class mainMenu:
    ...
    def main_funct(self): #function that gets input and checks what alternative to execute
        ...
        if(self.alt == "add"): #add new person by name and number
            if self.book.addPerson(self.inp[1], self.inp[2]): #parameters name, number
        ...
        if(self.alt == "lookup"): #lookup a person by serching for hims
            ...
            for p in res:
                ...
                for a in p.getAlias():
                    self.aliases += a+", "
                print "Name: " + p.getName() + " \tAlias: " + self.aliases + " \tNumber: " + p.getNumber() #result found, give it a nice printout
            return
        ...
        print self.inp[0] + " is not a valid command, type ? for help"
