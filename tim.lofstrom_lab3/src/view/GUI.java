	...
	public GUI(Controller c){
		...
		hostHameButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					controller.hostGame(Integer.parseInt(portField.getText()));
				} catch (IOException e1) {
					JOptionPane.showMessageDialog(null, "Refused to connect");
				}catch (NumberFormatException exe){
					JOptionPane.showMessageDialog(null, "Please fill in port");
				}
			}
		});
		...
		connectButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try{
					...
					controller.joinGame(ipField.getText(), Integer.parseInt(portField.getText()));	
		...
		serverListbox.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent arg0) {
				String val = serverListbox.getSelectedValue();
				if(val != null){
					String[] a = val.split(":");
					ipField.setText(a[0]);
		...
		if(message != null){
			if(message.getClass() == Vector.class){
				...
				if((caller == controller) && (message != null)){
					if(message.toString().contains("disconnected")){
						connectButton.setEnabled(true);
						hostHameButton.setEnabled(true);
						disconnectButton.setEnabled(false);
						refreshServersButton.setEnabled(true);
					}else if(message.toString().contains("connected")){
